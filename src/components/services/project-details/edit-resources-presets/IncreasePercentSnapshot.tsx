...
export function increasePercentSnapshotSubmitFormat(
    presetData: {
        [key: string]: unknown;
    },
...
// eslint-disable-next-line react-refresh/only-export-components
export function increasePercentSnapshotNote(presetData: {
    [key: string]: unknown;
}): string {
    return (
        "Increased snapshot resources by " +
        ("percent" in presetData ? presetData["percent"] : "25") +
        "%"
    );
}

export default function IncreasePercentSnapshot({
    resourceValues,
    resources,
    clusters,
    updatePresetData,
    ...
    resourceValues = resourceValues.filter((rv: ResourceValue) =>
        resources
            .filter((r: Resource) => r.resource_type === "snapshot")
            .map((r: Resource) => r.id)
